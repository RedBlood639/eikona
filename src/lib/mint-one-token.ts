...
export const mintOneToken = async (wallet: any): Promise<string> => {
  const endpoint: any =
  ...
  const idl: any = await Program.fetchIdl(CANDY_MACHINE_PROGRAM_ID, provider);
  const anchorProgram = new Program(idl, CANDY_MACHINE_PROGRAM_ID, provider);
  const a: any = process.env.REACT_APP_CANDY_MACHINE_CACHE_PROGRAM_CONFIG;
  const config = new web3.PublicKey(a);
  const b: any = process.env.REACT_APP_CANDY_MACHINE_CACHE_PROGRAM_UUID;
  const [candyMachine] = await getCandyMachine(config, b);
